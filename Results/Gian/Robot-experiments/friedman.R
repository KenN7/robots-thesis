...
#R <- R[c(1,3,2,6,7,5,4),]
#rownames(R) <- 1:nrow(R)
R$algo <- factor(R$algo, levels = R$algo[c(1,3,2)])
print(R)
...
fried_test <- ggplot(R, aes(x=y,y=algo,x)) +
  #geom_point(size=2) +
  labs(x=paste("rank in ", "Reality", sep=""), y="Design Method") +
  ...
  scale_y_discrete(breaks=c("EvoCom", "Gianduja", "Chocolate"),
                   labels=c("\\texttt{EvoCom}", "\\texttt{Gianduja}", "\\texttt{Chocolate}")) +
