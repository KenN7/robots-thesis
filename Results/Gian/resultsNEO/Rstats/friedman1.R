...
friedman <- function(data1, data2, data3, suffix, title) {
  ...
  dataall <- dataall[dataall$alg == 'Chocolate' | dataall$alg == 'EvoCom' | dataall$alg == 'GiandujaE' | dataall$alg == 'Gianduja',]
  ...
  R$algo <- factor(R$algo, levels = R$algo[c(1,2,3,4)])
  print(R)
  ...
  fried_test <- ggplot(R, aes(x=y,y=algo,x)) +
    ...
    scale_y_discrete(breaks=c("EvoCom", "Gianduja", "GiandujaE", "Chocolate"),
                     labels=c("\\texttt{EvoCom}", "\\texttt{Gianduja}", "\\texttt{GiandujaE}", "\\texttt{Chocolate}")) +
