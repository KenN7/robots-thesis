...
friedman <- function(data1, data2, data3, suffix, title) {
  ...
  names <- c("EvoCom2PR", "EvoCom3PR", "Gianduja2PR", "Gianduja2EPR", "Gianduja3PR", "Gianduja3EPR", "EvoCom3XPR", "EvoCom2XPR")
  if (suffix == "PR") {
    dataall <- dataall[sapply(dataall$alg,function(x)endsWith(as.character(x),"PR")),]
  }
  else if (suffix =="sim") {
    dataall <- dataall[sapply(dataall$alg,function(x)!endsWith(as.character(x),"PR")),]
    names <- c("EvoCom2", "EvoCom3", "Gianduja2", "Gianduja2E", "Gianduja3", "Gianduja3E", "EvoCom3X", "EvoCom2X")
  }
