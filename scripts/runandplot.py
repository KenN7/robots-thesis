...
CONF = {
    "Aggregation":
        ( "automodegianduja_aggregation.xml", "/home/ken/depots/neat-argos3/optimization/expAGG/results-evo-agg227", "/home/ken/depots/argos3-Automode/optimization/aggreg-200k-0208exp-results.txt" ),
    "Decision making":
        ( "automodegianduja_decision.xml",
        "/home/ken/depots/neat-argos3/optimization/expDEC/results-evo-dec227",
        "/home/ken/depots/argos3-Automode/optimization/decision-results.txt" ),
    "Stop":
        ( "automodegianduja_stop.xml",
        "/home/ken/depots/neat-argos3/optimization/expDEC/results-evo-dec227",
        "/home/ken/depots/argos3-Automode/optimization/stop-results.txt" ),
    "Aggregation 2 Spots":
...
def write_res(evo, auto, task):
    print('opening score file')
    f = open('scores-%s.txt' % task, 'w')
    f.write('"res" "alg" "task"\n')
    i = 0
    for res in evo:
        f.write('"%i" "%s" "Evostick" "%s"\n' % (i,res,task))
        i+=1
    for res in auto:
        f.write('"%i" "%s" "AutoMode-Gianduja" "%s"\n' % (i,res,task))
        i+=1
...
#new stuff --------------
def extract_res_evo(name):
    t = re.compile('%s-(\d+)' % name)
    os.mkdir('results-evo-%s' % name)
    for i in os.listdir():
        if t.match(i):
            y = t.match(i)
            print(i)
...
def main(expfile, genfolder, autopfsm, task):
    #evo
    #args = p.parse_args()
    ...
    for i,l in enumerate(f):
        ...
        scoreauto.append(sc)
        print(sc)
    write_res(scoreevo, scoreauto, task)
...
if __name__ == "__main__":
    for item in CONF:
        print(item)
        main(CONF[item][0],CONF[item][1],CONF[item][2],item)
